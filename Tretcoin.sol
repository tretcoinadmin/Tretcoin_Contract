...
/**
 * @title ICOable
 * @dev Base contract which allows children to implement an ICO mechanism.
 */
contract ICOable is Ownable, Pausable {
 	event IcoTurnedON();
	event IcoTurnedOff();

	bool public IcoFlag = false;

	/**
	 * @dev Modifier to make a function callable only when the ICO is off.
	*/
	modifier whenIcoOff() {
	  require(!IcoFlag);
	  _;
	...
	/**
	 * @dev Modifier to make a function callable only when the ICO is on.
	 */
	modifier whenIcoOn() {
	  require(IcoFlag);
	  _;
  	...
  	/**
   	* @dev called by the owner to start ICO, triggers stopped state
   	*/
  	function turnOnICO() onlyOwner whenNotPaused whenIcoOff public {
    	IcoFlag = true;
    	emit IcoTurnedOff();
  	}

  	/**
   	* @dev called by the owner to stop ICO, returns to normal state
   	*/
  	function turnOffICO() onlyOwner whenNotPaused whenIcoOn public {
    	IcoFlag = false;
    	emit IcoTurnedOff();
  	}
}
...
contract Tretcoin is ERC20Interface, Ownable, FundsFreezable, Pausable, ICOable {
	...
	mapping (address => uint) public investedAmountOf; //How much ETH each address has invested
	...
	constructor(address _companyAddress, uint256 _rate) public {
		require(_companyAddress != 0x0);
	...
	// ICO functionality. Not in use for now. Scope for future.
	function () external payable whenIcoOn {
        createTokens();
    }

	// ICO functionality. Not in use for now. Scope for future. // onlyWhitelisted() is debatable
	function createTokens() public payable whenNotPaused whenIcoOn unFrozenAccount(msg.sender){
	    require(msg.value > 0);
	    ...
	    uint256 tokens = msg.value.mul(rate); // Decide no. of tretcoins to give

	    require(supply >= tokens && balances[owner] >= tokens); // Supply and owner balance check

	    balances[msg.sender] = balances[msg.sender].add(tokens); // Increase balance of sender
	    balances[owner] = balances[owner].sub(tokens); // Decrease from global balances

	    companyAddress.transfer(msg.value); // Transfer received ether to this company owned ETH address
	...
	*/
	...
	function setNewRate(uint256 _newRate) onlyOwner public returns(bool success) {
